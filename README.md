...
上海交通大学东方 Project 同人活动「东方南堂界域 V —— 被隐匿的交通大学 / Hidden SJTU in Four Seasons」的线上互动系统。
...
| 抽奖权重：贡献阶梯、区域排名加成、防重复中奖 | `lottery.json` |
...
测试覆盖：注册码一次性 / 重复注册 / 昵称冲突、互动结算与幂等、能量恢复、区域解锁链与 CLEAR、抽奖权重（贡献阶梯 + 区域排名 + 管理员调整）、奖池解锁与防重复中奖、节目事件、管理操作鉴权、服务重启持久化、审计日志文件容错。
